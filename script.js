...
FADE OUT.`;
  ...
  const history = {
    ...
    updateInput() {
      ...
      if (fountainInput.value === '') setPlaceholder(); else clearPlaceholder();
      this.updateButtons();
    },
    updateButtons() {
      document.querySelectorAll('#undo-btn, #undo-btn-mobile').forEach(btn => btn.disabled = !this.canUndo());
      document.querySelectorAll('#redo-btn, #redo-btn-mobile').forEach(btn => btn.disabled = !this.canRedo());
    }
  };
  ...
  function clearPlaceholder() {
    if (fountainInput.classList.contains('placeholder')) {
      fountainInput.value = '';
  ...
  function setupEventListeners() {
    ...
    safeAddListener('scene-navigator-btn', 'click', () => {
    ...
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') sceneNavigatorPanel.classList.remove('open');
    });
  ...
  function createNewProjectObject() {
    return {
      fileVersion: "1.0",
      projectInfo: { projectName: "Untitled", directorName: "", prodName: "Author", currency: "USD", scriptContent: "" },
      scenes: [],
  ...
  function parseScriptToUniversalFormat(scriptText, projectInfo) {
    ...
    output.tokens.forEach(token => {
      if (token.type === 'scene_heading') {
        ...
        const typeAndSetting = headingParts.trim();
        const time = (headingParts[10] || 'DAY').trim();
  ...
  async function saveAsPdfWithUnicode() {
    ...
    const output = fountain.parse(getFilteredScriptText(), true);
    screenplayOutput.innerHTML = `<div class="title-page"><h1>${projectData.projectInfo.projectName}</h1><p>${projectData.projectInfo.prodName}</p></div>${output.html.body}`;
    await doc.html(screenplayOutput, {
      callback: (doc) => { doc.save(`${projectData.projectInfo.projectName}.pdf`); },
  ...
  function renderScript() {
    ...
    if (showSceneNumbers) {
      let sceneCount = 0;
      const sceneHeadings = output.tokens.filter(t => t.type === 'scene_heading');
      scriptHtml = scriptHtml.replace(/<h3/g, () => {
        if (sceneHeadings[sceneCount]) {
          const sceneNum = sceneHeadings[sceneCount].scene_number;
          sceneCount++;
          return `<h3>${sceneNum}. `;
        }
        return `<h3>`;
  ...
  function handleActionBtn(e) {
    ...
    switch (action) {
      case 'caps': {
        ...
        newText = (currentLine === currentLine.toUpperCase()) ? currentLine.toLowerCase() : currentLine.toUpperCase();
        fountainInput.setRangeText(newText, lineStart, selectionStart);
        break;
      }
      case 'parens':
  ...
  function cycleText(options) {
    ...
    for (let i = 0; i < options.length; i++) {
      if (currentLine.includes(options[i])) { currentIndex = i; break; }
    }
    ...
    if (currentIndex > -1) {
      ...
      document.execCommand('insertText', false, nextOption);
    }
  }
  ...
  function openProjectInfoModal() {
    ...
    document.getElementById('prod-name-input').value = info.prodName || '';
    document.getElementById('director-name-input').value = info.directorName || '';
  }

  function handleSaveProjectInfo() {
    projectData.projectInfo.prodName = document.getElementById('prod-name-input').value;
    projectData.projectInfo.directorName = document.getElementById('director-name-input').value;
  ...
  function openTitlePageModal() {
    document.getElementById('title-page-modal').classList.add('open');
  ...
  function getFilteredScriptText() {
    ...
    const titleBlock = /^(?!INT\.|EXT\.|INT\.\/EXT\.)/im.test(sceneBlocks) ? sceneBlocks.shift() : '';
    ...
    sceneBlocks.forEach(block => {
      ...
      if (headingMatch) {
        ...
        const sceneNum = fountain.parse(heading, true).tokens.scene_number;
  ...
  function editSceneFromCard(sceneId) {
    const scenes = parseScriptToUniversalFormat(fountainInput.value, projectData.projectInfo).scenes;
    const sceneToFind = scenes.find(s => s.sceneId === sceneId);
    if (!sceneToFind) return;
    const textToFind = `${sceneToFind.sceneType}. ${sceneToFind.sceneSetting} - ${sceneToFind.dayNight}`;
    const index = fountainInput.value.toUpperCase().indexOf(textToFind);
    if (index > -1) {
      switchView('write');
      fountainInput.focus();
      ...
      alert("Could not find the scene in the editor. It may have been changed.");
    }
  }
  ...
  createModalHTML('project-info-modal', 'Project Info',
    `<div class="form-group"><label for="prod-name-input">Production Name / Title</label><input type="text" id="prod-name-input"></div>
     <div class="form-group"><label for="director-name-input">Author / Writer</label><input type="text" id="director-name-input"></div>`,
  ...
  createModalHTML('title-page-modal', 'Title Page',
    `<div class="form-group"><label for="title-input">Title</label><input type="text" id="title-input"></div>
     <div class="form-group"><label for="author-input">Author</label><input type="text" id="author-input"></div>`,
    `<button id="save-title-btn" class="main-action-btn">Save</button>`);

  async function shareScript() {
    if (navigator.share) {
      try {
        await navigator.share({ title: projectData.projectInfo.projectName, text: fountainInput.value });
  ...
  function toggleSceneNumbers() {
    ...
    updateSceneNoIndicator();
    saveProjectData();
    if (scriptView.classList.contains('active')) { renderScript(); }
  }

  function updateSceneNoIndicator() {
    const indicator = document.getElementById('scene-no-indicator');
    if (showSceneNumbers) { indicator.classList.add('on'); indicator.classList.remove('off'); }
    else { indicator.classList.add('off'); indicator.classList.remove('on'); }
  }
  ...
  function toggleAutoSave() {
    ...
    if (autoSaveInterval) {
      ...
      indicator.classList.add('off'); indicator.classList.remove('on');
      alert('Auto-save disabled.');
    } else {
      autoSaveInterval = setInterval(saveProjectData, 120000);
      indicator.classList.add('on'); indicator.classList.remove('off');
  ...
  function updateSceneNavigator() {
    ...
    sceneList.innerHTML = output.tokens
      .filter(t => t.type === 'scene_heading')
      .map((token) => `<li data-line="${token.line}">${token.text}</li>`).join('');
  ...
  // NEW: define initialize and call it
  function initialize() {
    loadProjectData();
