document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupEventListeners() {
        ...
        document.addEventListener('click', (e) => { 
            if (menuPanel.classList.contains('open') && !menuPanel.contains(e.target) && 
                e.target.id !== 'hamburger-btn' && !e.target.closest('#hamburger-btn')) { 
        ...
        // Scene and navigation listeners
        ...
        document.querySelectorAll('.action-btn').forEach(btn => {
            ...
            console.log(`Added action listener to button: ${btn.dataset.action}`); // Debug log
        });
        
        // Undo/Redo listeners
        document.querySelectorAll('#undo-btn, #undo-btn-mobile').forEach(btn => {
            btn.addEventListener('click', () => history.undo());
        });
        document.querySelectorAll('#redo-btn, #redo-btn-mobile').forEach(btn => {
        ...
        // Modal and other click listeners
    ...
    function parseScriptToUniversalFormat(scriptText, projectInfo) {
        // Check if fountain is available
        if (typeof fountain === 'undefined') {
            console.error('Fountain parser not loaded');
            return createNewProjectObject();
        }
        ...
        const defaultSequence = { 
            ...
            id: Date.now(), 
            name: "Main Sequence", 
            sceneIds: universalData.scenes.map(s => s.sceneId) 
        };
    ...
    function renderScript() { 
        ...
        if (typeof fountain === 'undefined') {
            console.error('Fountain parser not loaded');
            screenplayOutput.innerHTML = '<p>Error: Fountain parser not loaded</p>';
            return;
        }
    ...
    function handleActionBtn(e) { 
        ...
        console.log(`Action button clicked: ${action}`); // Debug log
        ...
        switch(action) { 
            ...
            case 'time': 
                ...
                break; 
            case 'transition': 
                cycleText(['CUT TO:', 'FADE IN:', 'FADE OUT.', 'DISSOLVE TO:']); 
                break; 
        } 
    ...
    function cycleText(options) { 
        document.execCommand('insertText', false, options[0]); 
    }
    ...
    function getFilteredScriptText() {
        ...
        scenes.forEach(scene => {
            if (category === 'sceneSetting' && scene.sceneSetting.toLowerCase().includes(value)) {
                filteredSceneNumbers.add(scene.sceneNumber);
            } else if (category === 'sceneType' && scene.sceneType.toLowerCase().replace(/\./g, '') === value.replace(/\./g, '')) {
                filteredSceneNumbers.add(scene.sceneNumber);
            } else if (category === 'cast' && scene.breakdownData.cast.some(c => c.name.toLowerCase().includes(value))) {
                filteredSceneNumbers.add(scene.sceneNumber);
            }
        });
        
        if (typeof fountain === 'undefined') {
            console.error('Fountain parser not loaded for filtering');
        ...
        output.tokens.forEach(token => {
            if(token.type === 'scene_heading') {
                include = filteredSceneNumbers.has(token.scene_number.toString());
            }
            if (include) {
                result.push(token.text);
                if (token.type === 'dialogue' || token.type === 'action' || token.type === 'parenthetical') {
                    result.push(''); // Add line breaks
                }
            }
        });
    ...
    async function shareSceneCardAsImage(cardElement) {
        ...
        try {
            ...
            if (window.Android && typeof window.Android.shareImageFromBase64 === 'function') {
                ...
                window.Android.shareImageFromBase64(base64Data, fileName);
            } else {
                canvas.toBlob((blob) => { 
    ...
    function toggleSceneNumbers() { 
        ...
        updateSceneNoIndicator(); 
        saveProjectData(); 
        if (scriptView.classList.contains('active')) { 
            renderScript(); 
        } 
    }
    ...
    function updateSceneNavigator() { 
        ...
        if (typeof fountain === 'undefined') {
            console.error('Fountain parser not loaded');
            sceneList.innerHTML = '<li>Error: Fountain parser not loaded</li>';
            return;
        }
        ...
        new Sortable(sceneList, { 
            ...
            ghostClass: 'dragging', 
            onEnd: (evt) => { 
                /* Reordering logic can be enhanced here */ 
            } 
        }); 
    }
